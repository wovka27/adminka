...
import { type IEstateListItem, fetchGetParkingList } from '@/services/REST/dom_admin/estate'
...
useAppHeader({
  ...
  breadcrumbs: [{ label: 'Объекты недвижимости' }, { label: 'Паркинги' }]
})

const { setList, data_list, handleChangeFilter, sort, meta } = useListView<IEstateListItem>({
  router,
  sort: true,
  meta: true,
  fetchGetList: fetchGetParkingList,
  filter: {
    complex: true,
    house: true
  },
...
<template>
  <div class="ParkingListView ListView">
    ...
    <PskTable :data="data_list" v-model:meta="meta" @update:meta="setList" v-model:sort="sort">
      <el-table-column label="Паркинг" sortable prop="name" min-width="300">
        <template #default="scope">
          <PskTableTdName
            :uid="scope.row.uid"
            :name="scope.row.name"
            :route="`/real_properties_parking/${scope.row.uid}`"
          />
        </template>
      </el-table-column>
      ...
      <el-table-column label="Этаж" sortable prop="floor" min-width="110">
        <template #default="scope">
          {{ scope.row.floor }}
        </template>
      </el-table-column>
      ...
      <el-table-column fixed="right" width="85">
        <template #default="scope">
          <TableActionsContainer>
            <PskLink small-font-weight :to="`/real_properties_parking/${scope.row.uid}`">Изменить</PskLink>
          </TableActionsContainer>
        </template>
      </el-table-column>
    </PskTable>
