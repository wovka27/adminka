...
const { is_data_loaded, apply, getIsStateBeforeEqualAfter } = useEditorView({
  ...
  setFormData: (form_data) => {
    ...
    is_image_order_change_allowed.value = Boolean(form_data.data.is_image_order_change_allowed)
    online_show.value = form_data.data.online_show || ''
    phone_number.value = form_data.data.phone_number || ''
    apartments.value = !!form_data.data.apartments
  },
  getRequestData: () => ({
...
const apartments = ref<boolean>(false)
    ...
    <PskGridContainer grid-column-count="3" grid-span="3">
      ...
      <PskGridContainer grid-span="3" grid-column-count="3" title="Общая информация">
        ...
        <PskSelect
          ...
          :options="refs.yandex_categories"
        />
