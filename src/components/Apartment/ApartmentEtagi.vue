...
const refs = useRefs(
  'cian-categories',
  'cian-decoration-types',
  'real-property-types',
  'real-property-square-types',
  'cian-replacement-room-count'
)
const { material_type_options, materials, getMaterials, attach, detach } = useUploadMaterials('etagi_real_property')
const { unloading_parameters, getRequestParameters, setParameters } = useUnloadingParameters()
...
const { is_data_loaded, apply, getIsStateBeforeEqualAfter } = useEditorView({
  ...
  getRequestData: () => ({
    ...
    is_apartments: is_apartments.value
  })
})
...
<template>
  <FormLayout v-if="is_data_loaded" :apply="apply">
    <PskGridContainer grid-column-count="3" grid-span="3">
      <FeedAggregatorAlertTitle type="etagi" />
      <UnloadingParameters
        aggregator_type="etagi"
